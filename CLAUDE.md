...
Top-level spec pages: `overview.md`, `mqtt.md`, `nodes.md`, `modules.md`, `components.md`, `versioning.md`

Message sub-pages (parent: `Messages`): `presence.md`, `status.md`, `command.md`, `result.md`, `alarm.md`, `channel.md`, `throttle.md`
...
| **Channel** | A pre-configured publishing channel for a status code, with rate/delta/aggregation settings |
| **Throttle** | A runtime start/stop control for a channel |
...
```
<node>/<type>/<code>[/<channel>][/<component>]
```

- `node_id` may be multi-level: `dk/cph/tlc-001`
- `type`: `status`, `command`, `alarm`, `result`, `presence`, `channel`, `throttle`
- `channel` is inserted between `code` and `component` for status topics only
